...
class DiffusionUnetHybridImagePolicy(BaseImagePolicy):
    ...
    def set_obstacle_guidance(self, center, radius, enabled=True):
        if center is None or radius is None or not enabled:
            ...
            return
        device = self.device
        self.obstacle_center = torch.as_tensor(center, dtype=torch.float32, device=device).view(1, 1, -1)
        self.obstacle_radius = torch.as_tensor(float(radius), dtype=torch.float32, device=device)
            ...
            if self.guidance_mode == "obstacle" and self.obstacle_center is not None and self.obstacle_radius is not None:
                ...
                if t < self.guidance_start_timestep:
                    ...
                    if obstacle_grad is None:
                        ...
                        if self.guidance_debug["none_grad_steps"] <= 3:
                            print("[guidance-warn] obstacle_grad is None; skipping this denoise step")
                    else:
                        grad_norm = float(torch.linalg.norm(obstacle_grad).detach().item())
                        if grad_norm <= 1e-12:
                            self.guidance_debug["zero_grad_steps"] += 1
                            if self.guidance_debug["zero_grad_steps"] <= 3:
                                print("[guidance-warn] obstacle_grad norm is zero; skipping this denoise step")
                        else:
